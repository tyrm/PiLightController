...
def thread_frame_maker(nf):
    ...
    time.sleep(2)
    nf.set(make_color_grid(8, 8, 127))


# LightWrite
# Compares new frame buffer to current buffer and updates lights
def thread_light_write(cf, nf):
    logging.info("starting LightWrite")

    # List configured devices
    for name in devices:
        logging.info("found device [{0}]".format(devices[name], ))
...
if __name__ == "__main__":
    ...
    for d in device_config.sections():
        if 'type' in device_config[d]:
            if device_config[d]['type'] == 'unicornhat':
                ...
                logging.debug("attempting to init Unicorn Hat [{0} bri:{1} rot:{2}]".format(d, rotation, brightness))
                UnicornHat(d, rotation, brightness)
            else:
                logging.warning("device {0} has unsupported type: {1}. ignoring.".format(d, device_config[d]['type']))
        else:
    ...
    tlw = threading.Thread(name='LightWrite',
                           ...
                           args=(current_frame_buffer, next_frame_buffer))
    tlw.start()
